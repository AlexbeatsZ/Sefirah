...
- Private-use glyphs in app-owned `FontIcon` controls can degrade to question marks when a content
  font is inherited. Do not hard-code the Windows-only `Segoe Fluent Icons` family in shared XAML:
  use `{StaticResource SymbolThemeFontFamily}` so WinUI resolves Segoe Fluent Icons while Uno/Skia
  resolves its bundled `Uno.Fonts.Fluent` asset; verify both platform builds separately.
