...
- [`docs/design/macos-app-lifecycle.md`](docs/design/macos-app-lifecycle.md): macOS login-startup source detection, LaunchAgent registration, status-item actions, close-to-background, Dock reopen, native bridge packaging, and verification. Read it before changing those lifecycle paths.
...
- `SMAppService.agent` requires an Apple-issued signing identity with a Team Identifier; the local
  persistent self-signed certificate has no Team Identifier and returns `kSMErrorInvalidSignature`.
  For this local macOS bundle, use the reconciled per-user LaunchAgent plus an embedded launcher
  that supplies `--login-startup`. Never launch the managed executable directly from launchd, or
  enabling the setting while the app is running can create a second instance.
- Uno/Skia `Window.Activate()` alone does not restore a miniaturized or ordered-out macOS window,
  and Uno 6.7 does not implement the AppKit reopen delegate. Restore through the AppKit bridge and
  keep the menu-bar action, Dock reopen, minimize recovery, and close-to-background on that path.
- macOS deployment process matching must accept command-line arguments after `Sefirah.Desktop`.
  Login-started instances include `--login-startup`; an executable-only end anchor misses them and
  can replace the bundle while the previous managed process remains alive.
