...
- On Uno/Skia, symbol glyphs are loaded from `ms-appx:///Uno.Fonts.Fluent/...`. `StorageFile.ResourcePathBase` can be initialized before the package entry assembly is registered; in the custom macOS wrapper that previously cached `Environment.CurrentDirectory` as `/`, so every symbol font load fell back to the system text font and rendered private-use glyphs as question marks. The native launcher must `chdir` to `Contents/Resources/runtime` before `exec`. Keep shared `FontIcon` styling on `SymbolThemeFontFamily` and verify the actual macOS process cwd after packaging changes.
